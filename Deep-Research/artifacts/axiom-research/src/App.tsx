...
function Shell({ children }: { children: ReactNode }) {
  ...
  return (
    <main className="w-full overflow-x-hidden min-h-[100dvh] bg-[var(--bg-surface)] text-[var(--ink)] selection:bg-[var(--tint-green)]">
      <div className="flex min-h-[100dvh]">
        <aside className={`${open ? 'fixed inset-y-0 left-0 z-40 flex w-[264px]' : 'hidden'} ${collapsed ? 'lg:w-[264px]' : 'lg:w-0 lg:overflow-hidden'} shrink-0 flex-col border-r border-[var(--line)] bg-[var(--bg-surface-soft)] transition-all duration-300 lg:relative lg:flex`}>
          ...
          <div className="px-4 pt-4"><Link href="/" data-testid="link-back-home" className="flex min-h-11 w-full items-center gap-2 rounded-lg border border-[var(--line)] bg-[var(--bg-chip)] px-3 py-2.5 text-[12px] font-medium text-[var(--body)] transition hover:border-[var(--accent-mid)] hover:text-[var(--accent)]"><ArrowLeft size={15} strokeWidth={1.8} /> Home</Link></div>
        ...
        <section className="min-w-0 flex-1">
          <header className="flex h-[76px] items-center justify-between border-b border-[var(--line)] bg-[var(--header-bg)] px-5 backdrop-blur sm:px-8">
            <div className="flex min-w-0 items-center gap-3"><button data-testid="button-toggle-sidebar" className="shrink-0 rounded-lg p-2 text-[var(--body)] hover:bg-[var(--bg-chip)]" onClick={() => { setOpen(true); setCollapsed(!collapsed); }}><Menu size={19} className="lg:hidden" /><PanelLeft size={17} className="hidden lg:block" /></button><span className="hidden shrink-0 text-[11px] text-[var(--muted)] sm:block">My workspace</span><span className="hidden shrink-0 text-[var(--muted-2)] sm:block">/</span><span className="min-w-0 truncate text-sm font-medium text-[var(--ink)] sm:text-base">{location.startsWith('/workspace/research/') ? 'Research workspace' : active}</span></div>
            <div className="flex shrink-0 items-center gap-2 sm:gap-3">
              ...
              <ThemeToggle className="shrink-0 text-[var(--body)] hover:bg-[var(--bg-chip)] hover:text-[var(--accent)]" />
              <DropdownMenu>
                <DropdownMenuTrigger data-testid="avatar-user" aria-label="Account menu" className="grid size-8 shrink-0 place-items-center rounded-full bg-[var(--tint-green)] text-[11px] font-semibold text-[var(--tint-green-text)] outline-none transition hover:ring-2 hover:ring-[var(--accent-mid)]">{initials}</DropdownMenuTrigger>
...
function NavItem({ href, icon: Icon, label, active }: { href: string; icon: typeof Focus; label: string; active: boolean }) {
  return <Link href={href} data-testid={`link-${label.toLowerCase().replaceAll(' ', '-')}`} className={`flex min-h-11 w-full items-center gap-3 rounded-lg px-3 py-2.5 text-left transition ${active ? 'bg-[var(--tint-green)] font-semibold text-[var(--accent)]' : 'text-[var(--body)] hover:bg-[var(--bg-chip)]'}`}><Icon size={16} strokeWidth={1.7} />{label}</Link>;
}

function PageFrame({ children, className = '' }: { children: ReactNode; className?: string }) { return <div className={`mx-auto max-w-[1280px] px-4 py-7 sm:px-8 lg:px-10 lg:py-9 ${className}`}>{children}</div>; }
...
function ResearchHeader({ item }: { item: Research }) {
  ...
  return <div className="mb-8 flex flex-col justify-between gap-5 xl:flex-row xl:items-end"><div><div className="mb-3 flex flex-wrap items-center gap-2"><span data-testid="status-research" className={`rounded-full px-2.5 py-1 font-mono text-[9px] uppercase tracking-[.15em] ${item.status === 'done' ? 'bg-[var(--tint-green)] text-[var(--tint-green-text)]' : paused ? 'bg-[var(--tint-amber)] text-[var(--tint-amber-text)]' : 'bg-[var(--tint-green)] text-[var(--tint-green-text)]'}`}>{item.status === 'done' ? 'Research complete' : paused ? 'Research paused' : `Research ${item.status}`}</span><span className="flex items-center gap-1 font-mono text-[9px] uppercase tracking-[.12em] text-[var(--muted)]"><Clock3 size={12} /> {item.elapsedMinutes} min elapsed</span></div><h1 data-testid="text-research-title" className="max-w-[760px] font-serif text-2xl leading-[1.05] tracking-[-.045em] text-[var(--ink)] sm:text-3xl md:text-5xl">{item.query}</h1><p className="mt-4 max-w-[700px] text-xs leading-[1.65] text-[var(--body)] sm:text-sm dark:text-slate-400">{item.summary || 'Axiom is mapping the evidence, testing claims, and building a source-backed answer.'}</p></div><div className="flex w-full flex-wrap items-center gap-2 xl:w-auto xl:shrink-0">{item.status === 'done' && <Button data-testid="button-download-pdf" className="w-full sm:w-auto" onClick={downloadPDF}><span className="flex items-center gap-2"><Download size={14} /> Download PDF</span></Button>}<Button data-testid="button-pause-research" className="w-full sm:w-auto" onClick={toggle} disabled={pause.isPending || item.status === 'done'}>{paused ? <span className="flex items-center gap-2"><Play size={14} /> Resume research</span> : <span className="flex items-center gap-2"><Pause size={14} /> Pause research</span>}</Button><Button data-testid="button-share-research" className="w-full sm:w-auto" primary onClick={share}>{copied ? <span className="flex items-center gap-2"><Check size={14} /> Link copied</span> : <span className="flex items-center gap-2"><ArrowUpRight size={14} /> Share workspace</span>}</Button></div></div>;
}
...
function History() {
  ...
  return <PageFrame><div className="border-b border-[var(--line)] pb-7"><Eyebrow>Research library</Eyebrow><h1 className="mt-2 font-serif text-2xl leading-none tracking-[-.05em] text-[var(--ink)] sm:text-3xl md:text-5xl">Your research trail</h1><p className="mt-3 max-w-full text-xs text-[var(--body)] sm:text-sm dark:text-slate-400">Every question, source, and checked claim in one private record.</p><div className="mt-6 flex flex-col justify-between gap-3 sm:flex-row sm:items-center"><div className="flex max-w-md items-center gap-2 rounded-xl border border-[var(--line)] bg-[var(--bg-elevated)] px-3 py-2.5"><Search size={14} className="text-[var(--muted)]" /><input data-testid="input-research-search" value={search} onChange={e => setSearch(e.target.value)} placeholder="Search by question or summary…" className="w-full bg-transparent text-[12px] text-[var(--ink)] outline-none placeholder:text-[var(--muted-2)]" />{search && <button data-testid="button-clear-search" onClick={() => setSearch('')}><X size={13} className="text-[var(--muted-2)] hover:text-[var(--accent)]" /></button>}</div><Link href="/workspace?new=1" data-testid="link-start-new-from-history" className="flex min-h-11 items-center justify-center gap-2 rounded-lg bg-[var(--accent)] px-3.5 py-2.5 text-[11px] font-medium text-[var(--on-accent)]"><Plus size={14} /> New research</Link></div><div className="mt-4 flex flex-wrap gap-2">{statuses.map(s => <button key={s.key} data-testid={`filter-status-${s.key}`} onClick={() => setStatusFilter(s.key)} className={`inline-flex min-h-11 items-center rounded-full px-3 py-1.5 font-mono text-[9px] uppercase tracking-[.1em] transition ${statusFilter === s.key ? 'bg-[var(--accent)] text-[var(--on-accent)]' : 'bg-[var(--bg-chip)] text-[var(--body)] hover:bg-[var(--bg-chip)]'}`}>{s.label}</button>)}</div></div>
...
function AuthRouter() { return <Switch><Route path="/" component={Landing} /><Route path="/login" component={LoginPage} /><Route path="/register" component={RegisterPage} /><Route path="/contact" component={ContactPage} /><Route path="/privacy" component={PrivacyPage} /><Route path="/terms" component={TermsPage} /><Route path="/docs" component={DocsPage} /><Route>{() => <RequireAuth><Shell><WorkspaceRouter /></Shell></RequireAuth>}</Route></Switch>; }
